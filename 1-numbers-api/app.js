...
console.log(`Running 1-numbers-api/app.js`);
...
function getFactForFavoriteNumber() {
  ...
  fetch(`${baseURL}${favNumber}?json`) // `?json` ensures the API returns the data in JSON format
    ...
    .catch(error => {
      ...
      document.getElementById('singleFact').innerHTML = `<p>Error: ${error.message}</p>`; // Display the error message on the page
    });
}
...
function getFactsForMultipleNumbers() {
  ...
  fetch(`${baseURL}${numbers.join(',')}?json`) // `numbers.join(',')` creates a comma-separated string of numbers
    ...
    .catch(error => {
      ...
      document.getElementById('multipleFacts').innerHTML = `<p>Error: ${error.message}</p>`; // Display the error on the page
    });
}
...
document.getElementById('getSingleFactBtn').addEventListener('click', getFactForFavoriteNumber);
document.getElementById('getMultipleFactsBtn').addEventListener('click', getFactsForMultipleNumbers);
document.getElementById('getFourFactsBtn').addEventListener('click', getMultipleFactsForFavoriteNumber);
